...
DefFunc _CreatePerlinMap2D
	FrameBegin ebx, esi
	NameParams %$BorderLen
	DefVars %$AndVal
	DefSizedVar %$VectorBuffer, 0x20

	invoke_cdecl _CreateBitMap, %$BorderLen, 2, 8
	mov ebx, eax

	mov dword %$AndVal, 0xFFFF
...
.blae1:
	...
	movss xmm7, %$AndVal
	shr ecx, 1
	shufps xmm5, xmm5, _MM_SHUFFLE(1, 0, 1, 0)
	shufps xmm7, xmm7, 0
.generate:
	movaps xmm0, xmm3
	pmuludq xmm0, xmm4
...
DefFunc _ConvertPerlinMapToAltitude
	...
	NameParams %$Ratio, %$Amplitude, %$SrcMap
	DefVars %$Steps, %$Reciprocal, %$Matrix
	DefVars %$X, %$Y, %$BX, %$BY, %$IX, %$IY
	DefSizedVar %$MatrixBuffer, 6 * 0x10 + 0x10
	...
	lea eax, [%$MatrixBuffer_Addr + 0x10]
	and eax, 0xFFFFFFF0
	mov %$Matrix, eax
	...
	movaps [%$_UV1], xmm0
	mov dword[%$_UV1M + Vector.z], __float32__(1.0)
...
.get_steps:
	cvtsi2ss xmm0, eax
	mulss xmm0, %$Reciprocal

	mov eax, __float32__(6.0)
	mov ecx, __float32__(15.0)
...
DefFunc _GenPerlinAltitude
	FrameBegin ebx, edi
	NameParams %$BorderLen, %$Ratio, %$Amplitude
	invoke_cdecl _CreatePerlinMap2D, %$BorderLen
	mov ebx, eax
	invoke_cdecl _ConvertPerlinMapToAltitude, %$Ratio, %$Amplitude, ebx
	mov edi, eax
...
DefFunc _GenPerlinLayerPoolProc
	FrameBegin ebx
	NameParams %$Param, %$CommonData, %$Index

	mov ebx, %$Param
	invoke_cdecl srand, %$Index
...
DefFunc _GenMultiLayerPerlinAltitude
	FrameBegin ebx, esi, edi
	NameParams %$Size, %$Altitude, %$NumLayers, %$Curve, %$ThreadPoolSize
...
.setjobs1:
	...
	GetAbsAddr ecx, _GenPerlinLayerPoolProc
	invoke_cdecl _PoolRun, ecx, NULL, %$ThreadPoolSize, %$NumLayers, %$Jobs, 0, 1
	mov edi, eax
.accumulate:
